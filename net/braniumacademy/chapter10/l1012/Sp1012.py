...
class LearnConfirmDialog(tk.Tk):
    ...
    def create_widgets(self):
        ttk.Button(text='Quit', command=self.btn_clicked). \
            grid(row=0, column=0, padx=4, pady=4)
        ttk.Button(text='Delete All', command=self.btn_delete_clicked). \
            grid(row=1, column=0, padx=4, pady=4)
        ttk.Button(text='Connect Database', command=self.btn_reconnect_clicked). \
            grid(row=2, column=0, padx=4, pady=4)

    def btn_clicked(self):
        title = 'Confirmation'
        message = 'Do you really want to quit?'
        ans = msgbox.askyesno(title, message)
        if ans:
            self.destroy()
    ...
    def btn_delete_clicked(self):
        ...
        ans = msgbox.askokcancel(title=title, message=message)
        if ans:
    ...
    def btn_reconnect_clicked(self):
        ...
        ans = msgbox.askretrycancel(title=title, message=message)
        if ans:
