class BankAcc:
    ...
    def __init__(self, acc_num, owner, start, end,
        ...
        self.__completed_testcase = acc_num
        self.__owner = owner
        self.__start = start
        self.__end = end
    ...
    def check_balance(self):
        ...
        if self.__status == 1:
            print(f'Account number: {self.__completed_testcase}')
            print(f'Owner: {self.__owner}')
            print(f'Balance: {self.__balance}đ')
        else:
    ...
    def transfer(self, other, amount):
        ...
        if self.__status == 1:
            if amount < self.__balance - BankAcc.DEFAULT_BALANCE:
                other.balance += amount
                self.__balance -= amount
                self.__total += amount  # Giả định hạn mức giao dịch chỉ áp dụng với chuyển tiền.
    ...
    def withdraw(self, amount, bank):
        """Rút tiền."""
        if self.__status == 1:
            if 0 < amount < self.__balance - BankAcc.DEFAULT_BALANCE \
                    and len(bank) > 0:
                self.__balance -= amount
                return amount
            else:
    ...
    def __str__(self):
        return f'acc_num={self.__completed_testcase}, owner={self.__owner}, ' \
               f'bank={self.__bank}, balance={self.__balance}, ' \
               f'start={self.__start}, end={self.__end}, ' \
               f'avg_balance={self.__avg_balance}, total={self.__total}, ' \
...
class DomesticCard(BankAcc):
    ...
    def withdraw(self, amount, bank):
        ...
        if result > 0 and self.__avg_balance < DomesticCard.BALANCE_NO_FEE:
            if bank == self.__bank:
                self.__balance -= DomesticCard.INTRA_FEE
                result += DomesticCard.INTRA_FEE
            else:
                self.__balance -= DomesticCard.INTER_FEE
    ...
    def __str__(self):
        return f'DomesticCard[{super().__str__()}, limit={self.__limit}]'
...
class VisaCard(BankAcc):
    ...
    def withdraw(self, amount, bank):
        ...
        if result > 0 and self.__avg_balance < VisaCard.BALANCE_NO_FEE:
            if bank == self.__bank:
    ...
    def pay_bill(self, bill, amount, inter=False):
        ...
        if inter is True and result > 0 and \
                self.__balance > self.__balance - BankAcc.DEFAULT_BALANCE:
            self.__balance -= self.__transaction_fee * VisaCard.EXCHANGE_RATE
        return result

    def transfer(self, other, amount):
        if self.__total < self.__limit:
    ...
    def __str__(self):
        return f'VisaCard[{super().__str__()}, anual_fee={self.__anual_fee}, ' \
               f'transaction_fee={self.__transaction_fee}, limit={self.__limit}, ' \
               f'uuid={self.__uuid}]'
